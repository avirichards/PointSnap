...
## September 6 — centered homepage preview

- User approved trying a centered search with the globe enlarged behind it. Built this direction in the isolated UI preview: one centered headline and supporting sentence, opaque foreground search panel, large globe fading into the background, and three compact route suggestion cards below. Removed the redundant On your radar readout and homepage toolbar caption. Source coverage and saved-search access remain available. Results keep their compact header.
- Globe follows the form route and retains rotation, grab-to-pause, continuing route trails, and release inertia. In this background placement it omits duplicate airport labels and marker selection; the exposed globe remains draggable and keyboard-rotatable, and the form owns route selection. This avoids tabbing to obscured airport buttons behind the opaque form. Other uses retain default globe labels. Reduced-motion preference remains respected.
- Phone uses a quieter globe and the full search form near the top. Choosing a suggestion focuses the updated form and brings it into view if needed. Route ideas remain explicitly separate from confirmed availability.
- Browser QA covers light/dark desktop and phone, city selection, date flexibility, party settings, route suggestions, keyboard focus, globe motion/drag/trails/inertia, and submission to the unchanged compact results flow. All search calls are intercepted; no live collectors or canonical runtimes touched. Evidence: `docs/evidence/centered-search-2026-09-06/`. This is a preview of the proposed homepage direction, awaiting user design feedback before a canonical handoff.
