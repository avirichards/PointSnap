...
| **B6_TRUEBLUE** | jetblue.com | **Fastly Varnish only** | T2 (Camoufox or curl_cffi) | T1 | required + SMS | booking.jetblue.com 200 | low | **EASY but NOT T0** — direct curl to `jbrest.jetblue.com/lfs-rwb/outboundLFS` returns 404 (verified 2026-05-19). Per awardwiz source: must page-nav `jetblue.com/booking/flights?…&usePoints=true` FIRST so the SPA establishes session, then capture the XHR to jbrest. Camoufox can do this; or curl_cffi if we can replicate the session cookies. |
