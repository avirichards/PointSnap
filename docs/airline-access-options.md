...
No new airline account, data subscription, commercial inquiry, OAuth application or connected session was created during this options review. Account connections are now in scope for evaluation; Delta has passed local application checks and one complete hosted Mac diagnostic; Smiles has passed local native and partner application searches; the remaining proposals do not change verified coverage until their own checks pass.

Smiles follow-up: no traveler login or subscription was needed for GRU–GIG (five flights / 42 fares for two adults) or GRU–CDG (40 / 280). This strengthens the app-operated anonymous-browser option. It does not establish native partner-program coverage, continuous hosted reliability, or access to every airline. Four further American tests using system WKWebView and fresh regular profiles remained denied or challenged; twelve local and seven hosted attempts are now recorded.
