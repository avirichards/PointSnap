...
**Broad search constraint:** the current server limit remains 20 searches per identity per 10 minutes. City × date windows can exceed it. The form explains the larger search, requests remain capped at two concurrently, and incomplete coverage remains visible. The September 6 appearance/calendar follow-up adds automatic browser-queue cooldown and retry for HTTP 429, honoring Retry-After or the current ten-minute quota window, with immediate cancellation through Stop search. Persistent server-side batching is still a stabilization item for the airline/search owner. This does not weaken the limit or claim full-city completeness after partial responses. See `docs/appearance-calendar-refinement.md`.
