...
async function main() {
  const [origin = "CPH", dest = "ARN", departDate = "2026-10-05", pax = "2"] =
    process.argv.slice(2);
  const query = parseQuery(
    new URLSearchParams({ origin, dest, departDate, pax, minCabin: "Y" }),
  );
  const runner = new SasBrowserRunner(),
    started = Date.now();
  try {
    ...
    await writeFile(
      "work/browser-probes/sas-latest.json",
      JSON.stringify(summary, null, 2) + "\n",
    );
    if (process.env.POINTSNAP_SAVE_PUBLIC_FIXTURE === "1")
      await writeFile(
        "work/browser-probes/sas-payload.json",
    ...
    console.log(
      JSON.stringify({
        ...
        elapsedMs: Date.now() - started,
        error:
          e instanceof BrowserSearchError ? e.message : "Sas probe failed.",
