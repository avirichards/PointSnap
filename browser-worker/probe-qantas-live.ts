...
async function main() {
  const [origin = "SYD", dest = "MEL", departDate = "2026-10-05", pax = "2"] =
    process.argv.slice(2);
  const query = parseQuery(
    new URLSearchParams({ origin, dest, departDate, pax, minCabin: "Y" }),
  );
  const runner = new QantasBrowserRunner(),
    started = Date.now();
  try {
    const r = await runner.search(query, AbortSignal.timeout(150000));
    const summary = {
      program: r.programId,
      query,
    ...
    await writeFile(
      "work/browser-probes/qantas-latest.json",
      JSON.stringify(summary, null, 2) + "\n",
    );
    if (process.env.POINTSNAP_SAVE_PUBLIC_FIXTURE === "1")
      await writeFile(
        "work/browser-probes/qantas-payload.json",
    ...
    console.log(
      JSON.stringify({
        ...
        elapsedMs: Date.now() - started,
        error:
          e instanceof BrowserSearchError ? e.message : "Qantas probe failed.",
