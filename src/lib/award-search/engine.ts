...
export async function runSearch(ids: string[], ctx: ProviderContext) {
  ...
  // Southwest and SAS searches are normally short. Start them before
  // longer browser collectors so their deadlines aren't consumed in their queue.
    ...
    while (index < singles.length && !ctx.signal.aborted) {
      ...
      try {
        ...
        if (browserIds.includes(id)) {
          source =
            id === "QR_PRIVILEGE"
              ? "Qatar Privilege Club · member airline browser"
              : id === "AF_FLYINGBLUE"
                ? "Flying Blue · member airline browser"
                : id === "VS_FLYING_CLUB"
                  ? "Virgin Atlantic Flying Club · member airline browser"
                  : id === "UA_MP"
